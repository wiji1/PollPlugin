...
public class RemoveCommand extends SubCommand {
	...
	public RemoveCommand() {
		super("remove", Permission.ADMIN.get());
	...
	public int execute(CommandSourceStack source, CommandContext<?> context) {
		...
		PollManager.getInstance().removePoll(pollId);

		source.getSender().sendMessage(Component.text("Poll with ID " + pollId + " has been removed successfully.", NamedTextColor.GREEN));
